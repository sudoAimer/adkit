...
网页主流程为「上传数据 → 选择算法 → 调整判定标准 → 对比结果」。第一步同时上传正常参考和验证图片；第二步点击“开始分析”，自动更新或复用参考库并检测验证图片。完成后进入阈值调整，第四步复用当前阈值和标注比较结果，可筛选判定不一致的图片。未标注数据不用于准确性统计。

各算法独立保存分数阈值。最小异常面积仅使用数值输入（px²，0 表示不筛选）。二值图白色表示原始像素分数大于等于当前阈值；“面积筛选后”仅保留达到最小面积的 8 连通区域。图像级判定仍需满足图像分数阈值。二值图由原始异常图实时生成，不重新运行模型。

新增 `POST /api/tasks/{id}/jobs/analyze`，请求体为 `{algorithms: [id, ...]}`；原有 fit/predict 接口保留。`GET /api/tasks/{id}/binary/{algorithm}/{image_id}?threshold=...&area_threshold=0` 返回当前数据版本的 PNG 二值图，缺少原始异常图时返回 404。
