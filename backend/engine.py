...
class DetectorEngine:
    ...
    def execute(self, task: dict, directory: Path, operation: str, progress) -> dict:
        ...
        try:
            ...
            if operation == "fit":
                model = create_detector(task["algorithm"], weights=self.weights[task["algorithm"]], device=self.device)
                self.fit(model, task, directory, progress)
                return {"model_ready": True}
            model = load_detector(task["algorithm"], directory / "model.pt", device=self.device,
                                  weights=self.weights[task["algorithm"]])
            self.predict(model, task, directory, progress)
            return {}
        finally:
            del model
            gc.collect()
    ...
    def fit(self, model, task: dict, directory: Path, progress) -> None:
        ...
        data = {"image_size": task["image_size"], "rotation": task["rotation"]}
    ...
    def predict(self, model, task: dict, directory: Path, progress) -> None:
        """逐图保存原图尺度结果；坏图单独报错，不丢失已完成的检测结果。"""
        import cv2
        import numpy as np
        from PIL import Image
        from adkit.data import prepare, read_rgb
        from adkit.anomalydino import render_map

        output = directory / "results"
        output.mkdir(exist_ok=True)
        for index, item in enumerate(task["test"]):
            result = {"id": item["id"], "name": item["name"], "original": item["url"]}
            try:
                rgb = read_rgb(directory / "images" / item["file"])
                batch = prepare(rgb, task["image_size"], model.patch_size,
