...
def test_offline_checkpoint_roundtrip_and_refit(tmp_path):
    ...
    with patch('socket.create_connection', side_effect=AssertionError('Unexpected network access')):
        detector = create_detector({'name':'anomalydino', 'weights':str(WEIGHTS), 'device':'cpu'})
        ...
        assert set(state) == {'format_version','algorithm','config','weight_sha256','memory_bank','metadata'}
