...
工作台双模型模式在同一份正常／待测图片及标注上运行 AnomalyDINO 和 SubspaceAD，采用相同输入几何与固定角度增强，各自建库和评分。表格按模型独立阈值统计图像级 FP/FN/TP/TN，未标注图片不计入这些指标。该流程不等同于下述历史官方协议；官方 YAML 使用 `data.alignment: legacy`，新工作台使用保留内容的向上补边。
